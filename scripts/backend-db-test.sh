...
# not refuse to start. The DATABASE_URL superuser handle stays available for
# fixtures and admin-only setup paths.
DB_HOST_PORT_PATH="${DATABASE_URL#postgres://postgres:postgres@}"
export TEST_DATABASE_URL="${TEST_DATABASE_URL:-postgres://billforge_app:${BILLFORGE_APP_PASSWORD}@${DB_HOST_PORT_PATH}}"
