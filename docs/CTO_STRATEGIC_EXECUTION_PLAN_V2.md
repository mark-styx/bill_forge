# Bill Forge: CTO Strategic Execution Plan
...
Bill Forge is a modular B2B SaaS platform targeting mid-market companies (50-500 employees) frustrated with either overbuilt enterprise AP tools or underpowered SMB solutions. This plan provides the technical blueprint for the 12-week MVP sprint.

### Current State Assessment

| Aspect | CEO Vision | Current Reality | Decision |
|--------|-----------|-----------------|----------|
| **Backend** | Rust (Axum) | Locust framework (Python) | Build Bill Forge in Rust |
| **Frontend** | Next.js 14+ | None | Build from scratch |
| **OLTP Database** | PostgreSQL | SQLite in Locust | PostgreSQL per-tenant |
| **Analytics** | DuckDB | Supported | DuckDB per-tenant |
| **Purpose** | Invoice Processing | AI Agent Orchestration | Repurpose Locust for Winston AI |

### Strategic Approach: Hybrid Build

1. **Bill Forge Core**: New Rust/Axum + Next.js 14+ codebase (12-week MVP)
2. **Winston AI**: Adapt Locust's LangGraph architecture (Phase 3, post-MVP)
3. **Architecture**: Database-per-tenant for complete data isolation
4. **OCR Strategy**: Local-first (Tesseract 5) with cloud fallback

### Key Technical Decisions

| Decision | Choice | Rationale |
|----------|--------|-----------|
| Architecture Pattern | Modular monolith | Start simple, split later if needed |
| Backend Language | Rust + Axum 0.7+ | Performance, memory safety, CEO preference |
| Frontend Framework | Next.js 14+ App Router | Modern React, RSC, CEO preference |
| Multi-tenancy | Database-per-tenant | Complete isolation, compliance-ready |
| Primary OCR | Tesseract 5 (local) | Privacy differentiator |
| OCR Fallback | AWS Textract | Accuracy for complex documents |
...
**Rationale:**
- Complete data isolation (regulatory compliance: healthcare, legal, finance)
...
|-----------|------------|-----------|
| OLTP Database | **PostgreSQL 16** | Per-tenant isolation, JSONB support, CEO preference |
...
│                                                                               │
│  Milestones:                                                                  │
│  M1 (W2): Auth Complete        M3 (W10): Workflow Functional                 │
...
*This technical plan is the consolidated strategic execution document for Bill Forge. It supersedes all previous versions and will be updated as decisions evolve based on pilot feedback.*
