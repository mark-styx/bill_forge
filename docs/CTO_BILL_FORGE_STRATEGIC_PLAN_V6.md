...
**Status:** Execution Plan - CEO Review
...
Bill Forge enters the AP automation market targeting an underserved segment: **mid-market companies (50-500 employees) processing 300-5,000 invoices monthly**. Enterprise solutions are overbuilt and overpriced. SMB tools lack workflow sophistication. We execute dramatically better on three dimensions: **speed, simplicity, and privacy**.
...
The existing Locust codebase (`/Users/mark/sentinel/locust/src/locust/`) provides a production-ready multi-agent AI framework with 69+ Python files that we'll adapt for Winston:

| Locust Component | Location | Bill Forge Use |
|------------------|----------|----------------|
| Agent orchestration | `workflows/`, `agents/` | Winston conversation flow |
| LLM backends (Claude + Ollama) | `llm/` | Winston model flexibility |
| Memory/embeddings | `memory/embeddings.py` | Semantic search over tenant data |
| Circuit breaker patterns | `ceo/errors.py` | Resilient OCR provider routing |
| Config management | `config.py` | Multi-tenant configuration |
| Persistence layer | `storage/`, `persistence/` | Reference for SQLite/PostgreSQL patterns |

**Implementation Note:** Locust is Python-based while Bill Forge core will be Rust. Winston will remain Python and consume Bill Forge APIs, creating a clean separation.
...
| Locust Component | File Location | Winston Adaptation |
|------------------|---------------|-------------------|
| Agent base classes | `agents/base.py` | Simplify for single-agent conversational use |
| LLM backend switching | `llm/claude.py`, `llm/ollama.py` | Keep Claude + Ollama support for flexibility |
| Memory/embeddings | `memory/embeddings.py` | Semantic search over tenant invoice/vendor data |
| Circuit breaker | `ceo/errors.py` | Resilient external API calls |
| Config management | `config.py` | Multi-tenant configuration |

**Components to Remove:**
- Software development agents (CTO, CPO, etc.) in `agents/executive.py`, `agents/planning.py`, `agents/execution.py`
- Code generation modules in `codegen/`
- Research workflows in `research/`
- Git integration in `git/`
- Engineering execution workflows in `workflows/engineering.py`
...
async def run_report(report_type: str, date_range: DateRange, tenant_id: str):
    ...
    Report types: summary, processing-times, queue-status, vendor-spend
    """
    # Calls bf-api: GET /api/v1/{tenant}/reports/{report_type}
    pass
...
async def approve_invoice(invoice_id: str, user_id: str, tenant_id: str):
    ...
    # Calls bf-api: POST /api/v1/{tenant}/invoices/{id}/approve
    pass

@tool
async def get_invoice_details(invoice_id: str, tenant_id: str):
    """Get full details of a specific invoice."""
    # Calls bf-api: GET /api/v1/{tenant}/invoices/{id}
    pass
```

### 11.3 Winston Architecture
...
|                                                                    |
|  +--------------------+    +--------------------+                  |
|  |   Chat Interface   |    |   Tool Registry    |                  |
|  |                    |    |                    |                  |
|  | - Conversation     |    | - search_invoices  |                  |
|  |   state management |    | - vendor_lookup    |                  |
|  | - Message history  |    | - run_report       |                  |
|  | - Claude/Ollama    |    | - approve_invoice  |                  |
|  |   model selection  |    | - get_invoice_...  |                  |
|  +--------------------+    +--------------------+                  |
|           |                         |                              |
|           v                         v                              |
|  +--------------------+    +--------------------+                  |
|  |  Tenant Context    |    |   API Client       |                  |
|  |                    |    |                    |                  |
|  | - Tenant ID from   |    | - Calls bf-api     |                  |
|  |   session          |    | - Auth via service |                  |
|  | - User permissions |    |   account token    |                  |
|  | - Semantic index   |    | - Rate limiting    |                  |
|  |   (from Locust     |    |                    |                  |
|  |   memory module)   |    |                    |                  |
|  +--------------------+    +--------------------+                  |
|                                                                    |
...
### 11.4 Winston Implementation Timeline (Phase 3+, Post-MVP)

| Week | Focus |
|------|-------|
| 1 | Fork Locust agent core, strip unused code, create winston/ service |
| 2 | Implement Bill Forge tools, API client integration |
| 3 | Chat UI component, testing, tenant isolation verification |
...
|---------|------|--------|---------|
| 1.0-5.0 | Jan-Feb 2026 | CTO | Initial drafts and iterations |
| 6.0 | Feb 2, 2026 | CTO | Consolidated with Locust codebase analysis, Winston integration plan |
