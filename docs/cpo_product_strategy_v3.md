# Bill Forge: CPO Product Strategy v3
...
Bill Forge enters the accounts payable automation market at a critical inflection point. The mid-market segment (50-500 employees) faces a painful gap: enterprise solutions (Coupa, SAP Ariba) are overbuilt and overpriced, while SMB tools (BILL, QuickBooks) lack the sophistication growing companies need.
...
| Rank | Bet | Hypothesis | Validation Metric | Target |
|------|-----|------------|-------------------|--------|
...
1. Manual data entry consumes 8+ hours/week [HIGH]
2. Approval bottlenecks—CEO travels, invoices wait [HIGH]
3. No visibility into cash flow commitments [MEDIUM]
4. Missed early payment discounts ($15K+ lost/year) [MEDIUM]
5. Audit anxiety—missing approvals, no trail [LOW]
...
Pain Points (Ranked)
1. Slow, clunky legacy system (Palette) [HIGH]
2. Poor OCR accuracy requiring manual rework [HIGH]
3. Expensive per-user licensing ($25K+/year) [MEDIUM]
...
Pain Points (Ranked)
1. No unified view across entities [HIGH]
2. Inconsistent approval processes per entity [HIGH]
3. Vendor consolidation across entities [MEDIUM]
...
| **MVP Priority** | Focus | Phase 2 | Phase 3 |
...
**Recommendation:** Launch with primary, A/B test #1 and #2 in paid acquisition channels.
...
| Feature | Module | User Story | Priority | Effort |
|---------|--------|------------|----------|--------|
| PDF/image invoice upload | Invoice Capture | As an AP clerk, I can upload invoices via drag-drop | P0 | M |
| OCR field extraction | Invoice Capture | As an AP clerk, I see auto-extracted vendor, invoice #, amount, date | P0 | H |
| Confidence scoring | Invoice Capture | As an AP clerk, I see which fields need review | P0 | M |
| Manual correction UI | Invoice Capture | As an AP clerk, I can fix OCR errors with inline editing | P0 | M |
| AP Queue | Invoice Capture | As an AP manager, I see invoices ready for approval | P0 | M |
| Error Queue | Invoice Capture | As an AP clerk, I see invoices needing manual entry | P0 | M |
| Vendor matching | Invoice Capture | As an AP clerk, I can match to existing vendors or create new | P0 | M |
| Amount-based routing | Invoice Processing | As a controller, I configure approval thresholds by dollar amount | P0 | H |
| Approve/Reject/Hold | Invoice Processing | As an approver, I can take action on invoices | P0 | M |
| **Email approvals** | Invoice Processing | As an approver, I can approve from inbox without login | P0 | H |
| Audit trail | Invoice Processing | As a controller, I see who approved what and when | P0 | M |
| User authentication | Platform | As a user, I can log in securely | P0 | M |
| Tenant isolation | Platform | As a customer, my data is completely isolated | P0 | H |
