...
fn test_session() -> BillComSession {
    BillComSession {
...
async fn test_429_retries_with_backoff() {
    ...
    let url = format!("{}/vendors", mock_server.uri());
    ...
    let client = BillComClient::new(
        test_session(),
        BillComEnvironment::Production,
        "test-dev-key".to_string(),
    );
...
/// Test that the client sends devKey and sessionId headers with timeout configured.
#[tokio::test]
async fn test_timeout_applied() {
    let mock_server = MockServer::start().await;
    let url = format!("{}/vendors", mock_server.uri());

    // Return 200 to verify the client works with timeout and correct headers
    Mock::given(method("GET"))
        .and(header("devKey", "test-dev-key"))
        .and(header("sessionId", "test-session-id"))
        .respond_with(ResponseTemplate::new(200).set_body_string("ok"))
        .expect(1)
        .mount(&mock_server)
        .await;
