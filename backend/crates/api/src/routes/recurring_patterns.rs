...
struct PatternRow {
    ...
    last_line_items_signature: Option<serde_json::Value>,
...
impl From<PatternRow> for RecurringPatternResponse {
    fn from(r: PatternRow) -> Self {
        Self {
            ...
            last_line_items_signature: r.last_line_items_signature,
            line_item_tolerance_pct: r.line_item_tolerance_pct,
...
async fn list_patterns(
    ...
    let rows = sqlx::query_as::<_, PatternRow>(
        r#"SELECT rp.id, rp.tenant_id, rp.vendor_id,
                  ...
                  rp.last_line_items_signature,
                  CAST(rp.line_item_tolerance_pct AS DOUBLE PRECISION) AS line_item_tolerance_pct,
