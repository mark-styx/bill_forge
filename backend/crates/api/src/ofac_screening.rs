...
impl OfacScreener {
    ...
    pub fn screen(&self, vendor_name: &str, dba: Option<&str>) -> OfacScreenOutcome {
        ...
        for entry in self.entries.iter() {
            ...
            if vendor_norm == primary_norm
                || dba_norm.as_ref().map_or(false, |d| *d == primary_norm)
            {
            ...
            for alias in &entry.aliases {
                ...
                if vendor_norm == alias_norm
                    || dba_norm.as_ref().map_or(false, |d| *d == alias_norm)
                {
            ...
            let dba_subset = dba_tokens
                .as_ref()
                .map_or(false, |dt| primary_tokens.is_subset(dt));
...
fn tokenize<'a>(normalized: &'a str) -> HashSet<&'a str> {
