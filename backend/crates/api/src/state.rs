...
impl AppState {
    pub async fn new(config: &Config) -> Result<Self> {
        ...
        let storage_path = std::path::Path::new(&config.tenant_db_path)
            .parent()
            .unwrap_or_else(|| std::path::Path::new("./data"));
        let storage: Arc<dyn StorageService> = Arc::new(
            LocalStorageService::new(storage_path)
                .await
        ...
        let redis = match &config.redis_url {
            Some(url) => match redis::Client::open(url.as_str()) {
                ...
                Err(e) => {
                    tracing::warn!(
                        "Failed to initialize Redis client, OCR will run synchronously: {}",
                        e
                    );
                    None
                }
    ...
    async fn backfill_edi_receiver_map(db: &Arc<DatabaseManager>) {
        ...
        let tenant_ids: Vec<(uuid::Uuid,)> =
            match sqlx::query_as("SELECT id FROM tenants WHERE is_active = true")
            ...
            if let Some((isa_id,)) = row {
                ...
                if let Ok(r) = result {
                    if r.rows_affected() > 0 {
                        backfilled += 1;
                        tracing::info!(
                            "Backfilled EDI receiver mapping: {} -> {}",
                            isa_id,
                            tenant_uuid
                        );
                    }
        ...
        if backfilled > 0 {
            tracing::info!(
                "EDI receiver map backfill complete: {} mappings added",
                backfilled
            );
        }
    }
