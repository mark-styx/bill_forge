...
async fn create_test_state() -> AppState {
    ...
    std::env::set_var("QUICKBOOKS_CLIENT_ID", "test_client_id");
    std::env::set_var("QUICKBOOKS_CLIENT_SECRET", "test_client_secret");
    std::env::set_var("QUICKBOOKS_REDIRECT_URI", "http://localhost:8080/api/v1/quickbooks/callback");
    std::env::set_var("QUICKBOOKS_ENVIRONMENT", "sandbox");
...
#[tokio::test]
async fn test_quickbooks_connect_requires_auth() {
...
#[tokio::test]
async fn test_quickbooks_disconnect_requires_auth() {
    let app = create_test_router().await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri("/api/v1/quickbooks/disconnect")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn test_quickbooks_status_requires_auth() {
    let app = create_test_router().await;
...
#[tokio::test]
async fn test_quickbooks_sync_vendors_requires_auth() {
    let app = create_test_router().await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri("/api/v1/quickbooks/sync/vendors")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn test_quickbooks_sync_accounts_requires_auth() {
    let app = create_test_router().await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri("/api/v1/quickbooks/sync/accounts")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn test_quickbooks_export_invoice_requires_auth() {
    let app = create_test_router().await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri("/api/v1/quickbooks/export/invoice/123")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
// ============================================================================
// QuickBooks Mappings Tests
// ============================================================================

#[tokio::test]
async fn test_quickbooks_get_account_mappings_requires_auth() {
    let app = create_test_router().await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::GET)
                .uri("/api/v1/quickbooks/mappings/accounts")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
#[tokio::test]
async fn test_quickbooks_update_account_mappings_requires_auth() {
    let app = create_test_router().await;
