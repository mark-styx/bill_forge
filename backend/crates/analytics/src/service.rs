...
impl AnalyticsService {
    ...
    pub async fn get_usage_summary(
        ...
        let total_events = self.repo.get_event_count(tenant_id, start_date, end_date).await?;
        let unique_users = self.repo.get_unique_user_count(tenant_id, start_date, end_date).await?;
        let top_features = self.repo.get_feature_usage(tenant_id, start_date, end_date).await?;
        let performance_metrics = self.repo.get_performance_metrics(tenant_id, start_date, end_date).await?;
    ...
    pub async fn get_daily_summary(&self, tenant_id: &str) -> Result<UsageSummary> {
        ...
        self.get_usage_summary(tenant_id, start_date, end_date).await
    }
    ...
    pub async fn get_trends(&self, tenant_id: &str) -> Result<Vec<TrendData>> {
        ...
        for metric in metrics {
            if let Ok(trend) = self.repo.get_trend_data(
                tenant_id,
                metric,
                current_start,
                now,
    ...
    pub async fn get_feature_usage(
        ...
        self.repo.get_feature_usage(tenant_id, start_date, end_date).await
    }
    ...
    pub async fn get_performance_metrics(
        ...
        self.repo.get_performance_metrics(tenant_id, start_date, end_date).await
    }
}
