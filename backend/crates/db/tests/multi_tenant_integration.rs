...
async fn test_tenant_isolation() {
    ...
    // Create tenants
    manager
        .create_tenant(&tenant1_id, "Test Tenant 1")
        .await
        .expect("Failed to create tenant 1");

    manager
        .create_tenant(&tenant2_id, "Test Tenant 2")
        .await
    ...
    // Clean up
    cleanup_tenant(&manager, &tenant1_id)
        .await
        .expect("Failed to cleanup tenant 1");
    cleanup_tenant(&manager, &tenant2_id)
        .await
        .expect("Failed to cleanup tenant 2");
