...
use crate::{
    domain::{
        ApprovalRequest, ApprovalStatus, ApprovalTarget, Invoice, ProcessingStatus, StageType,
        WorkflowRule, WorkflowTemplate, WorkflowTemplateStage,
    },
    services::{EmailAction, EmailActionTokenService},
    traits::{
        ApprovalRepository, InvoiceRepository, UserRepository, WorkQueueRepository,
    ...
    pub async fn process_invoice_workflow(
        ...
        if let Some(ref template_repo) = self.template_repo {
            if let Some(template) = template_repo.get_default(tenant_id).await? {
                let stopped_stage = self
                    .process_invoice_through_template(tenant_id, invoice, &template)
                    .await?;
        ...
        if !rule_matched && invoice.processing_status == ProcessingStatus::Draft {
            ...
            if let Ok(Some(default_queue)) = self.queue_repo.get_default(tenant_id).await {
                self.queue_repo
                    .move_item(tenant_id, &invoice.id, &default_queue.id, None)
                    .await?;
            }
            self.invoice_repo
                .update(
    ...
    pub async fn process_invoice_through_template(
        ...
        for stage in &stages {
            ...
            if !stage.auto_advance_conditions.is_empty()
                && crate::workflow_evaluator::evaluate_conditions(
                    invoice,
        ...
        match action.action_type {
            ...
            ActionType::IntelligentRoute => {
                let routing_provider = self.routing_provider.as_ref().ok_or_else(|| {
                    Error::Validation(
                        "IntelligentRoute requires a routing data provider".to_string(),
                    )
                })?;
...
mod tests {
    ...
    impl crate::traits::WorkflowTemplateRepository for MockTemplateRepository {
        ...
        async fn get_default(&self, _tenant_id: &TenantId) -> Result<Option<WorkflowTemplate>> {
    ...
    > {
        let pool =
            std::sync::Arc::new(sqlx::PgPool::connect_lazy("postgres://localhost/test").unwrap());
    ...
    async fn test_process_invoice_workflow_template_skip_conditions_skip_stage() {
        ...
        let template = crate::domain::WorkflowTemplate {
            ...
            stages: vec![
                ...
                make_template_stage(1, "Approval", Some(queue_id_2.clone()), vec![], vec![]),
            ],
            created_at: Utc::now(),
            updated_at: Utc::now(),
        };
    ...
    async fn test_intelligent_route_selects_best_approver() {
        ...
        assert!(
            result.is_ok(),
            "IntelligentRoute should succeed: {:?}",
            result
        );
    }
    ...
    async fn test_intelligent_route_fallback_no_approvers() {
        ...
        assert!(
            result.is_err(),
            "IntelligentRoute with no approvers should fail"
        );
